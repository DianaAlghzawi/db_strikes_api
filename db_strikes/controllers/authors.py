...
async def insert(auther: Author) -> JSONResponse:
    return JSONResponse(content=jsonable_encoder(authors.new(auther.bio)), status_code=status.HTTP_201_CREATED)


@authors_router.get('/{id}')
async def get_by_id(id: UUID) -> JSONResponse:
    return JSONResponse(content=jsonable_encoder(authors.get_by_id(id)), status_code=status.HTTP_200_OK)
...
async def update(id: UUID, patch_author: PatchAuthor) -> JSONResponse:
    return JSONResponse(content=jsonable_encoder(authors.update(id, patch_author.bio)), status_code=status.HTTP_200_OK)
